...
public interface CacheSerializer {
    ...
    static JavaType getJavaType(Type type) {
        ...
        if (type instanceof ParameterizedType) {
            ...
            return TypeFactory.createDefaultInstance().constructParametricType(rowClass, javaTypes);
        } else {
            // 简单类型直接用该类构建 JavaType
            Class<?> cla = (Class<?>) type;
            return TypeFactory.createDefaultInstance().constructParametricType(cla, new JavaType[0]);
        }
    }
