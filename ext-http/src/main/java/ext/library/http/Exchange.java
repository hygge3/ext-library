...
public class Exchange {
	...
	public <R> R onResponse(@NotNull Function<ResponseSpec, R> func) {
	...
	public <R> R onResponse(@NotNull Function<ResponseSpec, R> func, BiFunction<Request, IOException, R> errFunc) {
		try (HttpResponse response = new HttpResponse(call.execute())) {
			return func.apply(response);
		}
		catch (IOException e) {
	...
	public <R> R onSuccess(@NotNull Function<ResponseSpec, R> func) {
		try (HttpResponse response = new HttpResponse(call.execute())) {
			return func.apply(response);
		}
		catch (IOException e) {
	...
	public <T> T atJsonPathValue(String jsonPtrExpr, Class<T> valueType) {
		return JsonUtil.convert(atJsonPath(jsonPtrExpr), valueType);
	}
