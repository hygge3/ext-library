...
public class InMemoryIdempotentKeyStore implements IdempotentKeyStore {
	...
	public InMemoryIdempotentKeyStore() {
		this.cache = Caffeine.newBuilder().expireAfter(new Expiry<String, Long>() {
			@Override
	...
	public synchronized boolean saveIfAbsent(String key, long duration, TimeUnit timeUnit) {
		...
		if (value == null) {
			cache.policy()
				.expireVariably()
				.ifPresent(e -> e.put(key, System.currentTimeMillis(), Duration.of(duration, timeUnit.toChronoUnit())));
			return true;
		}
		return false;
	}
