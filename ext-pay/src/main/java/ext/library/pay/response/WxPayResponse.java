...
public class WxPayResponse {
	...
	public static WxPayResponse of(Map<String, String> res) {
		return JsonUtil.readObj(JsonUtil.toJson(res), WxPayResponse.class).setRaw(res);
	}

	/**
	 * 返回状态码。此字段是通信标识，非交易标识，交易是否成功需要查看 result_code 来判断
	 */
	...
	/**
	 * 预支付交易会话标识。微信生成的预支付回话标识，用于后续接口调用中使用，该值有效期为 2 小时
	 */
