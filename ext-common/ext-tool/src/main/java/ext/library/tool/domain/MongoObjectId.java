 ...
 * MongoDB 对象 ID 工具类
 * <p>
 * 提供 MongoDB ObjectId 的生成，校验和转换功能。该类用于生成符合 MongoDB 规范的 ObjectId 字符串，
 * 支持从字节数组转换为十六进制字符串，以及从十六进制字符串转换为字节数组。
 * <p>
 * ObjectId 由 12 字节组成，包含时间戳，机器标识，进程 ID 和随机数等信息。
 * 该类通过结合当前时间戳和随机值生成唯一的 ObjectId.
 *
 * @since 2025.10.24
 */
